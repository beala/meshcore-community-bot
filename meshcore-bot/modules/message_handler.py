...
class MessageHandler:
    ...
    def __init__(self, bot):
        ...
        # Entries are (value, timestamp) tuples; evicted after rf_data_timeout
        self.snr_cache = {}
        self.rssi_cache = {}
        self._cache_max_size = 500  # Hard cap to prevent unbounded growth
        ...
        try:
            ...
            if snr == 'unknown':
                ...
                if pubkey_prefix and pubkey_prefix in self.snr_cache:
                    snr = self.snr_cache[pubkey_prefix][0]  # (value, timestamp) tuple
            ...
            if rssi == 'unknown':
                ...
                if pubkey_prefix and pubkey_prefix in self.rssi_cache:
                    rssi = self.rssi_cache[pubkey_prefix][0]  # (value, timestamp) tuple
        ...
        try:
            ...
            if 'snr' in payload:
                ...
                if packet_prefix and snr_value is not None:
                    # Cache the SNR value with timestamp for eviction
                    self.snr_cache[packet_prefix] = (snr_value, time.time())
                ...
                if 'rssi' in payload:
                    ...
                    if packet_prefix and rssi_value is not None:
                        # Cache the RSSI value with timestamp for eviction
                        self.rssi_cache[packet_prefix] = (rssi_value, time.time())
                ...
                if packet_prefix:
                    ...
                    # Evict expired entries from SNR/RSSI caches
                    for cache in (self.snr_cache, self.rssi_cache):
                        expired_keys = [k for k, (_, ts) in cache.items()
                                        if current_time - ts > self.rf_data_timeout]
                        for k in expired_keys:
                            del cache[k]
                        # Hard cap: drop oldest entries if cache is too large
                        if len(cache) > self._cache_max_size:
                            sorted_keys = sorted(cache, key=lambda k: cache[k][1])
                            for k in sorted_keys[:len(cache) - self._cache_max_size]:
                                del cache[k]
    ...
    async def process_message(self, message: MeshMessage):
        ...
        if 'stats' in self.bot.command_manager.commands:
            ...
            if stats_command:
                await stats_command.record_message_async(message)
                await stats_command.record_path_stats_async(message)
            ...
            if greeter_command and greeter_command.should_execute(message):
                try:
                    ...
                    if 'stats' in self.bot.command_manager.commands:
                        stats_command = self.bot.command_manager.commands['stats']
                        if stats_command:
                            await stats_command.record_command_async(message, 'greeter', response_sent)
                except Exception as e:
        ...
        # Discord webhook forwarding for incoming messages
        if message.channel == "#bot":
            await discord_webhook.send_to_discord(
                self.bot.config.get("Discord", "bot_webhook_url", fallback=""),
                message.sender_id or "Unknown",
                message.content,
                is_incoming=True
            )
        elif message.channel == "#emergency":
            await discord_webhook.send_to_discord(
                self.bot.config.get("Discord", "emergency_webhook_url", fallback=""),
                message.sender_id or "Unknown",
                message.content,
                is_incoming=True
            )
            # Rebroadcast emergency to configured broadcast channel (two messages)
            broadcast_channel = self.bot.config.get("Discord", "emergency_broadcast_channel", fallback="")
            if broadcast_channel:
                await self.bot.command_manager.send_channel_message(
                    broadcast_channel, "🚨 EMERGENCY MESSAGE FROM #EMERGENCY 🚨"
                )
                await self.bot.command_manager.send_channel_message(
                    broadcast_channel, f"{message.sender_id}: {message.content}"
                )
            ...
            for keyword, response in keyword_matches:
                ...
                if 'stats' in self.bot.command_manager.commands:
                    ...
                    if stats_command:
                        ...
                        await stats_command.record_command_async(message, keyword, True)
