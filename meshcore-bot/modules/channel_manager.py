...
class ChannelManager:
    ...
    async def fetch_all_channels(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        ...
        await asyncio.to_thread(self._store_channels_in_db, valid_channels)
    ...
    async def add_channel(self, channel_idx: int, channel_name: str, channel_secret: Optional[bytes] = None, channel_secret_hex: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if hasattr(self.bot.meshcore, 'commands') and hasattr(self.bot.meshcore.commands, 'set_channel'):
                ...
                await asyncio.to_thread(self._store_single_channel_in_db, channel_info)
        ...
        try:
            ...
            try:
                ...
                if channel_set:
                    ...
                    if result:
                        self._channels_cache[channel_idx] = result
                        await asyncio.to_thread(self._store_single_channel_in_db, result)
                        self.logger.info(f"Successfully added channel {channel_idx}: {channel_name}")
                        return True
                    else:
        ...
        try:
            ...
            try:
                ...
                if channel_cleared:
                    ...
                    try:
                        def _delete_channel(db_path, idx):
                            with sqlite3.connect(db_path) as conn:
                                cursor = conn.cursor()
                                cursor.execute('DELETE FROM channels WHERE channel_idx = ?', (idx,))
                                conn.commit()
                        await asyncio.to_thread(_delete_channel, self.bot.db_manager.db_path, channel_idx)
                    except Exception as e:
