...
class FeedManager:
    ...
    async def poll_all_feeds(self):
        ...
        try:
            ...
            feeds = await asyncio.to_thread(self._get_enabled_feeds)
    ...
    async def poll_feed(self, feed: Dict[str, Any]):
        ...
        try:
            ...
            await asyncio.to_thread(self._update_feed_last_check, feed_id)

        except Exception as e:
            self.logger.error(f"Error polling feed {feed_id}: {e}")
            await asyncio.to_thread(self._record_feed_error, feed_id, 'network', str(e))
    ...
    async def process_rss_feed(self, feed: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                db_processed_ids = await asyncio.to_thread(self._get_processed_item_ids, feed['id'])
                processed_item_ids.update(db_processed_ids)
            except Exception as e:
            ...
            if items:
                ...
                await asyncio.to_thread(self._update_feed_last_item_id, feed['id'], all_items[-1]['id'])
    ...
    async def _send_feed_item(self, feed: Dict[str, Any], item: Dict[str, Any]):
        ...
        try:
            ...
            await asyncio.to_thread(self._queue_feed_message, feed, item, message)
        except Exception as e:
            self.logger.error(f"Error processing feed item: {e}")
            await asyncio.to_thread(self._record_feed_error, feed['id'], 'other', str(e))
    ...
    def _get_processed_item_ids(self, feed_id: int) -> set:
        """Get all processed item IDs for a feed from the feed_activity table"""
        processed_ids = set()
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def _get_unsent_messages(self) -> List[Dict[str, Any]]:
        """Get all unsent messages from the feed message queue"""
        with sqlite3.connect(self.db_path) as conn:
    ...
    def _mark_queue_message_sent(self, queue_id: int):
    ...
    async def process_message_queue(self):
        ...
        try:
            ...
            messages = await asyncio.to_thread(self._get_unsent_messages)
            ...
            for msg in messages:
                ...
                try:
                    ...
                    if success:
                        # Mark as sent
                        await asyncio.to_thread(self._mark_queue_message_sent, queue_id)

                        # Record activity
                        await asyncio.to_thread(self._record_feed_activity, feed_id, item_id, item_title)
                        ...
                        await asyncio.to_thread(self._record_feed_error, feed_id, 'channel', f"Failed to send to channel {channel_name}")
                        # Don't mark as sent, will retry later

                except Exception as e:
                    self.logger.error(f"Error sending queued feed message: {e}")
                    await asyncio.to_thread(self._record_feed_error, feed_id, 'other', str(e))
                    # Don't mark as sent, will retry later
