...
class StatsCommand(BaseCommand):
    ...
    def record_message_sync(self, message: MeshMessage):
        """Record a message in the stats database (sync DB operation)"""
    ...
    def record_message(self, message: MeshMessage):
        """Record a message in the stats database (backward-compatible sync call)"""
        self.record_message_sync(message)

    async def record_message_async(self, message: MeshMessage):
        """Record a message in the stats database (async wrapper)"""
        await asyncio.to_thread(self.record_message_sync, message)
    
    def record_command_sync(self, message: MeshMessage, command_name: str, response_sent: bool = True):
        """Record a command execution in the stats database (sync DB operation)"""
    ...
    def record_command(self, message: MeshMessage, command_name: str, response_sent: bool = True):
        """Record a command execution in the stats database (backward-compatible sync call)"""
        self.record_command_sync(message, command_name, response_sent)

    async def record_command_async(self, message: MeshMessage, command_name: str, response_sent: bool = True):
        """Record a command execution in the stats database (async wrapper)"""
        await asyncio.to_thread(self.record_command_sync, message, command_name, response_sent)
    
    def record_path_stats_sync(self, message: MeshMessage):
        """Record path statistics for longest path tracking (sync DB operation)"""
        if not self.stats_enabled or not self.track_all_messages:
            return
    ...
    def record_path_stats(self, message: MeshMessage):
        """Record path statistics for longest path tracking (backward-compatible sync call)"""
        self.record_path_stats_sync(message)

    async def record_path_stats_async(self, message: MeshMessage):
        """Record path statistics for longest path tracking (async wrapper)"""
        await asyncio.to_thread(self.record_path_stats_sync, message)
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            ...
            if self.auto_cleanup:
                await self.cleanup_old_stats_async(self.data_retention_days)
            ...
            await self.record_command_async(message, 'stats', True)
    ...
    def _get_basic_stats_sync(self) -> str:
        """Get basic bot statistics (sync DB operation)"""
        try:
...
{self.translate('commands.stats.basic.top_user', user=top_user)}"""
    ...
    async def _get_basic_stats(self) -> str:
        """Get basic bot statistics (async wrapper)"""
        return await asyncio.to_thread(self._get_basic_stats_sync)
    
    def _get_bot_user_leaderboard_sync(self) -> str:
        """Get leaderboard for bot users (sync DB operation)"""
        try:
            # Get time window (24 hours ago)
            now = int(time.time())
            day_ago = now - (24 * 60 * 60)
    ...
    async def _get_bot_user_leaderboard(self) -> str:
        """Get leaderboard for bot users (async wrapper)"""
        return await asyncio.to_thread(self._get_bot_user_leaderboard_sync)
    
    def _get_channel_leaderboard_sync(self) -> str:
        """Get leaderboard for channel message activity (sync DB operation)"""
        try:
            # Get time window (24 hours ago)
            now = int(time.time())
            day_ago = now - (24 * 60 * 60)
    ...
    async def _get_channel_leaderboard(self) -> str:
        """Get leaderboard for channel message activity (async wrapper)"""
        return await asyncio.to_thread(self._get_channel_leaderboard_sync)
    
    def _get_path_leaderboard_sync(self) -> str:
        """Get leaderboard for longest paths seen (sync DB operation)"""
        try:
            # Get time window (24 hours ago)
            now = int(time.time())
            day_ago = now - (24 * 60 * 60)
    ...
    async def _get_path_leaderboard(self) -> str:
        """Get leaderboard for longest paths seen (async wrapper)"""
        return await asyncio.to_thread(self._get_path_leaderboard_sync)
    
    def cleanup_old_stats_sync(self, days_to_keep: int = 7):
        """Clean up old stats data to prevent database bloat (sync DB operation)"""
        try:
    ...
    def cleanup_old_stats(self, days_to_keep: int = 7):
        """Clean up old stats data (backward-compatible sync call)"""
        self.cleanup_old_stats_sync(days_to_keep)

    async def cleanup_old_stats_async(self, days_to_keep: int = 7):
        """Clean up old stats data (async wrapper)"""
        await asyncio.to_thread(self.cleanup_old_stats_sync, days_to_keep)

    def cleanup_old_daily_stats_sync(self, days=90):
        """Clean up old daily_stats entries (sync DB operation)"""
        try:
            from datetime import datetime, timedelta
            ...
            with sqlite3.connect(self.bot.db_manager.db_path) as conn:
                ...
                conn.commit()
                if deleted > 0:
                    self.logger.info(f'Cleaned up {deleted} old daily_stats entries (older than {days} days)')
        except Exception as e:
            self.logger.error(f'Error cleaning up old daily stats: {e}')

    def cleanup_old_daily_stats(self, days=90):
        """Clean up old daily_stats entries (backward-compatible sync call)"""
        self.cleanup_old_daily_stats_sync(days)

    async def cleanup_old_daily_stats_async(self, days=90):
        """Clean up old daily_stats entries (async wrapper)"""
        await asyncio.to_thread(self.cleanup_old_daily_stats_sync, days)

    def cleanup_all_tables_sync(self):
        """Master cleanup for all stats tables with tiered retention (sync DB operation)"""
        try:
            # Core stats tables (30 days, uses data_retention_days from config)
            self.cleanup_old_stats_sync(self.data_retention_days)

            # Daily stats (90 days)
            self.cleanup_old_daily_stats_sync(90)

            # Auxiliary tables with varied retention
            with sqlite3.connect(self.bot.db_manager.db_path) as conn:
                cursor = conn.cursor()

                cursor.execute("DELETE FROM feed_activity WHERE processed_at < datetime('now', '-30 days')")
                feed_act = cursor.rowcount

                cursor.execute("DELETE FROM feed_errors WHERE occurred_at < datetime('now', '-30 days')")
                feed_err = cursor.rowcount

                cursor.execute("DELETE FROM channel_operations WHERE created_at < datetime('now', '-14 days')")
                chan_ops = cursor.rowcount

                cursor.execute("DELETE FROM feed_message_queue WHERE queued_at < datetime('now', '-7 days')")
                feed_q = cursor.rowcount

                conn.commit()

                total = feed_act + feed_err + chan_ops + feed_q
                if total > 0:
                    self.logger.info(f'Auxiliary cleanup: {feed_act} feed_activity, {feed_err} feed_errors, {chan_ops} channel_ops, {feed_q} feed_queue')

        except Exception as e:
            self.logger.error(f'Error in cleanup_all_tables: {e}')

    def cleanup_all_tables(self):
        """Master cleanup for all stats tables (backward-compatible sync call)"""
        self.cleanup_all_tables_sync()

    async def cleanup_all_tables_async(self):
        """Master cleanup for all stats tables (async wrapper)"""
        await asyncio.to_thread(self.cleanup_all_tables_sync)

    
    def get_stats_summary_sync(self) -> Dict[str, Any]:
        """Get a summary of all stats data (sync DB operation)"""
        try:
            with sqlite3.connect(self.bot.db_manager.db_path) as conn:
                cursor = conn.cursor()
    ...
    def get_stats_summary(self) -> Dict[str, Any]:
        """Get a summary of all stats data (backward-compatible sync call)"""
        return self.get_stats_summary_sync()

    async def get_stats_summary_async(self) -> Dict[str, Any]:
        """Get a summary of all stats data (async wrapper)"""
        return await asyncio.to_thread(self.get_stats_summary_sync)
