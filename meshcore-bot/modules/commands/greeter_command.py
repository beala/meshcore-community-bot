...
class GreeterCommand(BaseCommand):
    ...
    def _get_recent_activity_count_sync(self) -> int:
        """Get recent activity count from message_stats (sync helper for _get_mesh_info)"""
        try:
            with sqlite3.connect(self.bot.db_manager.db_path) as conn:
                cursor = conn.cursor()
                # Check if message_stats table exists
    ...
    async def _get_mesh_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if info['recent_activity_24h'] == 0:
                info['recent_activity_24h'] = await asyncio.to_thread(self._get_recent_activity_count_sync)
    ...
    def _check_greeting_freshness_sync(self, sender_id: str, channel: str) -> Optional[float]:
        """
        Check how recently a user was marked as greeted (sync helper for execute).
        ...
        Returns:
            seconds_ago (float) if record found, None if not found or on error
        """
        try:
            with sqlite3.connect(self.bot.db_manager.db_path) as conn:
                cursor = conn.cursor()
                ...
                if result:
                    ...
                    return (now - greeted_at).total_seconds()
        except Exception:
            return None
        return None
    ...
    async def execute(self, message: MeshMessage) -> bool:
        ...
        try:
            ...
            if self.has_been_greeted(message.sender_id, message.channel):
                ...
                try:
                    seconds_ago = await asyncio.to_thread(
                        self._check_greeting_freshness_sync, message.sender_id, message.channel
                    )
