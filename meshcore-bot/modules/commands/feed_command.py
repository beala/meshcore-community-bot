...
feed status 1"""
    ...
    async def _handle_subscribe(self, message: MeshMessage, args: List[str]) -> bool:
        ...
        # Create subscription
        try:
            feed_id = await self._acreate_subscription(
                feed_type=feed_type,
                feed_url=feed_url,
    ...
    async def _handle_unsubscribe(self, message: MeshMessage, args: List[str]) -> bool:
        ...
        try:
            ...
            try:
                ...
                success = await self._adelete_subscription_by_id(feed_id)
            except ValueError:
                # Try as URL
                if channel_name:
                    success = await self._adelete_subscription_by_url(identifier, channel_name)
                else:
    ...
    async def _handle_list(self, message: MeshMessage, args: List[str]) -> bool:
        ...
        try:
            feeds = await self._aget_subscriptions(channel_filter)
    ...
    async def _handle_status(self, message: MeshMessage, args: List[str]) -> bool:
        ...
        try:
            feed_id = int(args[0])
            feed = await self._aget_subscription_by_id(feed_id)
    ...
    async def _handle_enable_disable(self, message: MeshMessage, args: List[str], enable: bool) -> bool:
        ...
        try:
            feed_id = int(args[0])
            success = await self._aset_subscription_enabled(feed_id, enable)
    ...
    async def _handle_update(self, message: MeshMessage, args: List[str]) -> bool:
        ...
        try:
            ...
            success = await self._aupdate_subscription(feed_id, interval)
    ...
    # --- Async wrappers for sync DB methods (run in thread to avoid blocking event loop) ---

    async def _acreate_subscription(self, feed_type: str, feed_url: str, channel_name: str,
                                    feed_name: Optional[str] = None, api_config: Optional[Dict] = None) -> int:
        """Async wrapper for _create_subscription"""
        return await asyncio.to_thread(self._create_subscription, feed_type, feed_url, channel_name, feed_name, api_config)

    async def _adelete_subscription_by_id(self, feed_id: int) -> bool:
        """Async wrapper for _delete_subscription_by_id"""
        return await asyncio.to_thread(self._delete_subscription_by_id, feed_id)

    async def _adelete_subscription_by_url(self, feed_url: str, channel_name: str) -> bool:
        """Async wrapper for _delete_subscription_by_url"""
        return await asyncio.to_thread(self._delete_subscription_by_url, feed_url, channel_name)

    async def _aget_subscriptions(self, channel_filter: Optional[str] = None) -> List[Dict]:
        """Async wrapper for _get_subscriptions"""
        return await asyncio.to_thread(self._get_subscriptions, channel_filter)

    async def _aget_subscription_by_id(self, feed_id: int) -> Optional[Dict]:
        """Async wrapper for _get_subscription_by_id"""
        return await asyncio.to_thread(self._get_subscription_by_id, feed_id)

    async def _aset_subscription_enabled(self, feed_id: int, enabled: bool) -> bool:
        """Async wrapper for _set_subscription_enabled"""
        return await asyncio.to_thread(self._set_subscription_enabled, feed_id, enabled)

    async def _aupdate_subscription(self, feed_id: int, interval: Optional[int] = None) -> bool:
        """Async wrapper for _update_subscription"""
        return await asyncio.to_thread(self._update_subscription, feed_id, interval)

    # --- Sync DB methods (kept for use via asyncio.to_thread) ---
