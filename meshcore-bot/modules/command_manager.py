...
class CommandManager:
    ...
    async def handle_advert_command(self, message: MeshMessage):
        ...
        if 'stats' in self.commands:
            ...
            if stats_command:
                await stats_command.record_command_async(message, 'advert', response_sent)
            ...
            if result:
                if hasattr(result, 'type') and result.type == EventType.ERROR:
                    ...
                    if isinstance(error_payload, dict) and error_payload.get('reason') == 'no_event_received':
                        ...
                        self.bot.rate_limiter.record_send()
                        self.bot.bot_tx_rate_limiter.record_tx()
                        # Discord webhook for bot responses (timeout case)
                        if channel == "#bot":
                            await discord_webhook.send_to_discord(
                                self.bot.config.get("Discord", "bot_webhook_url", fallback=""),
                                self.bot.config.get("Bot", "bot_name", fallback="MeshBot"),
            ...
            if command.should_execute(message):
                ...
                if not command.can_execute_now(message):
                    ...
                    if 'stats' in self.commands:
                        stats_command = self.commands['stats']
                        if stats_command:
                            await stats_command.record_command_async(message, command_name, response_sent)
                ...
                if command.requires_internet:
                    ...
                    if not has_internet:
                        ...
                        if 'stats' in self.commands:
                            stats_command = self.commands['stats']
                            if stats_command:
                                await stats_command.record_command_async(message, command_name, True)
                        return
                ...
                try:
                    ...
                    if 'stats' in self.commands:
                        stats_command = self.commands['stats']
                        if stats_command:
                            await stats_command.record_command_async(message, command_name, True)  # Error message counts as response
