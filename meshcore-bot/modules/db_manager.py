...
class DBManager:
    ...
    async def aget_cached_geocoding(self, query: str) -> Tuple[Optional[float], Optional[float]]:
        return await asyncio.to_thread(self.get_cached_geocoding, query)

    async def acache_geocoding(self, query: str, latitude: float, longitude: float, cache_hours: int = 720):
        return await asyncio.to_thread(self.cache_geocoding, query, latitude, longitude, cache_hours)

    async def aget_cached_value(self, cache_key: str, cache_type: str) -> Optional[str]:
        return await asyncio.to_thread(self.get_cached_value, cache_key, cache_type)

    async def acache_value(self, cache_key: str, cache_value: str, cache_type: str, cache_hours: int = 24):
        return await asyncio.to_thread(self.cache_value, cache_key, cache_value, cache_type, cache_hours)

    async def aget_cached_json(self, cache_key: str, cache_type: str) -> Optional[Dict]:
        return await asyncio.to_thread(self.get_cached_json, cache_key, cache_type)

    async def acache_json(self, cache_key: str, cache_value: Dict, cache_type: str, cache_hours: int = 720):
        return await asyncio.to_thread(self.cache_json, cache_key, cache_value, cache_type, cache_hours)

    async def acleanup_expired_cache(self):
        return await asyncio.to_thread(self.cleanup_expired_cache)

    async def acleanup_geocoding_cache(self):
        return await asyncio.to_thread(self.cleanup_geocoding_cache)

    async def aget_database_stats(self) -> Dict[str, Any]:
        return await asyncio.to_thread(self.get_database_stats)

    async def avacuum_database(self):
        return await asyncio.to_thread(self.vacuum_database)

    async def acreate_table(self, table_name: str, schema: str):
        return await asyncio.to_thread(self.create_table, table_name, schema)

    async def adrop_table(self, table_name: str):
        return await asyncio.to_thread(self.drop_table, table_name)
    ...
    async def aexecute_update(self, query: str, params: Tuple = ()) -> int:
        return await asyncio.to_thread(self.execute_update, query, params)

    async def aset_metadata(self, key: str, value: str):
        return await asyncio.to_thread(self.set_metadata, key, value)

    async def aget_metadata(self, key: str) -> Optional[str]:
        return await asyncio.to_thread(self.get_metadata, key)

    async def aget_bot_start_time(self) -> Optional[float]:
        return await asyncio.to_thread(self.get_bot_start_time)

    async def aset_bot_start_time(self, start_time: float):
        return await asyncio.to_thread(self.set_bot_start_time, start_time)
