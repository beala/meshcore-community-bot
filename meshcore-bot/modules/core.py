...
class MeshCoreBot:
    ...
    def __init__(self, config_file: str = "config.ini"):
        ...
        # Connection watchdog - tracks last event from radio to detect silent failures
        self._last_event_time = time.time()
        self._reconnecting = False
        self._watchdog_timeout = 1800  # 30 minutes with no events triggers reconnect
...
"""
    ...
    def _touch_event_time(self):
        """Update last event time - called on any received radio event"""
        self._last_event_time = time.time()
    ...
    async def setup_message_handlers(self):
        ...
        # Handle disconnect events for immediate reconnection
        async def on_disconnect(event, metadata=None):
            self.logger.warning(f"Received DISCONNECTED event: {event.payload}")
        ...
        # Subscribe to DISCONNECTED events for immediate reconnection
    ...
    async def _reconnect(self):
        """Tear down and rebuild the meshcore connection"""
        if self._reconnecting:
            return
        self._reconnecting = True
        try:
            self.logger.warning("Initiating reconnection to MeshCore node...")
            ...
            for attempt in range(1, 6):
                self.logger.info(f"Reconnection attempt {attempt}/5...")
            ...
            self.logger.error("All reconnection attempts failed, stopping bot")
            self.connected = False
        finally:
            self._reconnecting = False
    ...
    async def start(self):
        ...
        self.logger.info(f"Connection watchdog active (timeout: {self._watchdog_timeout}s)")
        ...
        try:
            while self.connected:
                ...
                # Connection watchdog - detect silent message reception failures
                if not self._reconnecting and self.meshcore:
                    elapsed = time.time() - self._last_event_time
                    if elapsed > self._watchdog_timeout:
                        self.logger.warning(
                            f"No radio events received in {int(elapsed)}s "
                            f"(threshold: {self._watchdog_timeout}s), triggering reconnect"
                        )
                        await self._reconnect()
                    # Also check if meshcore library reports disconnected
                    elif not self.meshcore.is_connected:
                        self.logger.warning("MeshCore reports disconnected, triggering reconnect")
                        await self._reconnect()
